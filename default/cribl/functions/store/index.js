...
let hadReset;     // indicator that a reset signal was received
  ...
  if (event.__signalEvent__) {
    switch (event.__signalEvent__) {
      ...
      case 'final':
        ...
        break;
      // pass 'reset' and 'complete_gen' aggregation signals through;
      // this ensures events are correctly appended to lookup table
      case 'reset':
        if (hadReset) {
          // as part of export operation, previews for aggregations are suppressed,
          // and hence we should only expect exactly one 'reset' signal
          logger.error(`Unexpected 'reset' signal received`);
        } else {
          logger.debug(`'reset' signal received`);
        }
        hadReset = true;
        break;
      case 'complete_gen':
