 ...
 * @returns CriblEvent
 */
function createStatsEvent(evt, destination, isFinal = false) {
  ...
  // nothing to report
  if (!isFinal && deltaBytesOut === 0 && deltaEventsOut === 0) return undefined;

  let status = 'Exporting';
  if (isFinal) {
    // if cancel request is received due to an error, e.g. lambda shutting down after 900s, send appropriate warning stat message
    status = evt.__ctrlFields.includes('cancel') ? 'Exporting stopped due to errors or cancellation' : 'Exporting complete';
  }
