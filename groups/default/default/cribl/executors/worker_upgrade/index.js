...
exports.taskExecute = async (job, opts) => {
  ...
  const pkg = opts.packages.find((p) => p.variant[0] === variant[0] && p.variant[1] === variant[1]);
  ...
  if (typeof upgradeClient.performUpgrade === 'function') {
    ...
    upgradeResult = await upgradeClient.performUpgrade(pkg, opts, logger);
    ...
    const descriptor = {
      packageUrl: pkg.localPackageUrl,
      hashUrl: pkg.localHashUrl,
      version: pkg.version,
    };
