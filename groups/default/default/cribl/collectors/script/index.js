...
exports.init = async (opts) => {
  ...
  envVars = {};
  (conf.envVars || []).forEach(obj => { envVars[obj.name] = C.expr.runExprSafe(obj.value) });
...
exports.discover = async (job) => {
  ...
  const env = {...envVars, EARLIEST: conf.earliest, LATEST: conf.latest };

  const spawnGen = new SpawnGenerator({ shell: shellCmd, cmd: conf.discoverScript, env });

  try {
    await scopedResource(
      pipe(spawnGen)
        .byLines()
        .map((line) => ({ host, source: line.trim(), format: 'raw' }))
...
exports.collect = async (collectible, job) => {
  ...
  proc.on('exit', code => {
    if (code) {
      ...
      job.logger().error(msg, { exitCode: code, source: collectible.source, error: lastErr.toString() });
