...
exports.init = (opts) => {
  ...
  if (conf.fields) {
    ...
    for (const curEntry of conf.fields) {
      if (curEntry && curEntry.inFieldName && curEntry.metricType) {
        new NestedPropertyAccessor(curEntry.inFieldName.trim()); // Using NPA to check for valid field names
        _metricsConf.values.push(curEntry.inFieldName.trim());
        const defaultOutFieldExpr = (curEntry.metricType === 'summary' || curEntry.metricType === 'histogram') ? '_metric' : undefined;
        curEntry.outFieldExpr = (curEntry.outFieldExpr) ? curEntry.outFieldExpr.trim() : defaultOutFieldExpr;
  ...
  cLogger.info('Function initialized', { overwrite, _metricsConf, _dimensionsToAddConf, _dimensionsToRemoveConf});
};
    ...
    for (let x = 0; x < event.__criblMetrics.length; x++) {
      ...
      for (let y = 0; y < metric.values.length; y++) {
        ...
        if (!_metricsToRemoveConf || !_metricsToRemoveConf.test(metric.values[y])) {
          if (metric.nameExpr != null) metricCopy.nameExpr.push(metric.nameExpr[y]);
          metricCopy.values.push(metric.values[y]);
          metricCopy.types.push(metric.types[y]);
        }
      }

      if (!metricCopy.values.length) {
        // removed metric
        continue;
      }
      ...
      if (_dimensionsToAddConf) {
        ...
        else if (_metricsConf == null) {
          // Add dimensions to existing metric.
          dimsCopy = Array.from(new Set([...dimsCopy, ..._dimensionsToAddConf]));
        }
        else if (_metricsConf && x === event.__criblMetrics.length-1) {
          // Adding new metric, only update the new metric & leave others as is.
          dimsCopy = [..._dimensionsToAddConf];
        } /* else use dimsCopy as is */
      }
